...
{{--                                <a class="mt-2 btn btn-danger">Delete</a>--}}
                                ...
                                <form action="{{route('bonuses.destroy', $item->id)}}" method="post">
                                    {{csrf_field()}}
                                    <a href="{{route('bonuses.edit',$item->id)}}" class="mt-2 btn btn-primary">Edit</a>
                                    @method('DELETE')
                                    <button onclick="return confirm('Are you sure?')" type="submit" class="mt-2 btn btn-danger">Delete</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#show_report_button").click(function () {
                var url = '/bonuses?fdate=' + $('#fdate').val() + '&tdate=' + $('#tdate').val();
